...
const AuthForm = () => {
  ...
  const submitHandler = async event => {
    ...
    setIsLoading(true);
    const { idToken, expiresIn } = {
      idToken: "somebullshittoken123",
      expiresIn: "1800",
    };
    const expirationTimestamp =
      new Date().getTime() + parseInt(expiresIn, 10) * 1000;
    authCtx.login(idToken, expirationTimestamp);
    navigate("/", { replace: true });

    let url;
    if (isLogin) {
      //if Login or signup (the same logic applies to both just dfferent URLs)
      url =
        "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyC10myv-0cDjhwdwT_i6T8URC3xUxfw6x8";
    } else {
      url =
        "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyC10myv-0cDjhwdwT_i6T8URC3xUxfw6x8";
    }
